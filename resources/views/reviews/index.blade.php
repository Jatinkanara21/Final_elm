...
@section('content')
<div class="bg-mocha-light min-h-screen py-12">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-16">
            <h1 class="text-4xl md:text-5xl font-serif font-bold text-mocha-text mb-4">Elm Grove Reviews</h1>
            <p class="text-lg text-gray-600 max-w-2xl mx-auto italic">Share your experience with our store and see what others are saying.</p>
            
            <div class="mt-8 flex flex-col items-center justify-center">
                <div class="flex text-yellow-500 mb-2">
                    @for($i=1; $i<=5; $i++)
                        <svg class="w-8 h-8 {{ $i <= round($avgRating) ? 'fill-current' : 'text-gray-300 fill-current' }}" viewBox="0 0 20 20">
                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path>
                        </svg>
                    @endfor
                </div>
                <p class="text-2xl font-bold text-mocha-text">{{ number_format($avgRating, 1) }} / 5.0</p>
                <p class="text-sm text-gray-500 uppercase tracking-widest mt-1">Average Store Rating</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div class="lg:col-span-1">
                <div class="glassmorphism p-8 rounded-2xl border border-white/20 shadow-xl bg-white/70 sticky top-24">
                    <h2 class="text-2xl font-serif font-bold text-mocha-text mb-6">Leave a Review</h2>
                    ...
                    @auth
                        <form action="{{ route('reviews.store') }}" method="POST" x-data="{ rating: 5, hoverRating: 0 }">
                            ...
                            <div class="mb-8">
                                <label class="block text-sm font-bold text-gray-700 mb-3 uppercase tracking-wide">Your Experience</label>
                                <textarea name="comment" rows="5" class="w-full bg-white border-gray-200 rounded-xl focus:ring-mocha-accent focus:border-mocha-accent text-gray-800 p-4 shadow-inner" placeholder="Tell us about your visit..." required></textarea>
                            </div>
            ...
            <div class="lg:col-span-2 space-y-8">
                @forelse($reviews as $review)
                    <div class="glassmorphism p-8 rounded-2xl border border-white/20 shadow-sm transition-all hover:shadow-md">
                ...
                @empty
                    <div class="text-center py-20 bg-white/50 rounded-2xl border border-dashed border-gray-300">
                        ...
                        <h3 class="text-xl font-medium text-gray-500 italic">No store reviews yet.</h3>
                        <p class="text-gray-400 mt-2">Personalize your visit with a first review!</p>
                    </div>
                @endforelse
